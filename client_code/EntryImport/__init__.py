...
class EntryImport(EntryImportTemplate):
  ...
  def EntryImport_change(self, file, **event_args):
    ...
    for row in rows[1:]:
      ...
      app_tables.entries.add_row(startYear=int(row_data['startYear']),endYear=int(row_data['endYear']),firstName=row_data['firstName'],lastName=row_data['lastName'],middleName=row_data['middleName'],occupation=row_data['occupation'],geoLocation=row_data['geoLocation'],content=content,created=datetime.now(),updated=datetime.now())
