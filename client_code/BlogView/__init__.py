from ._anvil_designer import BlogViewTemplate
from anvil import *
import anvil.server
...
class BlogView(BlogViewTemplate):
  ...
  def edit_entry_button_click(self, **event_args):
    ...
    save_clicked = alert(
      content=BlogEdit(item=entry_copy),
      title="Update Blog",
      large=True,
    ...
    if save_clicked:
      anvil.server.call("update_blog", self.item, entry_copy)
