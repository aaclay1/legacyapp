from ._anvil_designer import BlogHomeTemplate
from anvil import *
import anvil.server
...
class BlogHome(BlogHomeTemplate):
  ...
  def add_entry_button_click(self, **event_args):
    ...
    save_clicked = alert(
      content=BlogEdit(item=new_entry),
      title="Add Blog",
      large=True,
    ...
    if save_clicked:
      anvil.server.call("add_blog", new_entry)
      self.refresh_entries()
  ...
  def delete_entry(self, entry, **event_args):
    # Delete the entry
    anvil.server.call("delete_blog", entry)
